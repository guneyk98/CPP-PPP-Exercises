...
/*
Modified from float version (Ex09)
...
class Token_stream {
public:
	Token_stream(std::istream& is) : in{is} {}
	Token get();
	void putback(Token t);
	void ignore(char c);
private:
	std::istream& in; //use any istream
	bool full = false;
	Token buffer;
};
...
void clean_up_this_mess(Token_stream& ts);
...
int main()
{
	try {
		...
		Token_stream ts{std::cin}; //choose std::cin
		calculate(ts);
		return 0;
	}
